# Codex — Deep Styles (Deep Reference: examples, curves, code, Before/After)

> **TL;DR.** Полный справочник Design Engineering Эмиля Ковальски: точные кривые easing, длительности, CSS/JS-сниппеты, spring-конфиги, clip-path-паттерны, правила производительности и таблицы Before/After/Why — всё VERBATIM. Обзорная версия с принципами и чек-листом — в [codex.design.deep-styles.md](codex.design.deep-styles.md).
>
> ⚠️ Большой файл (~600 строк, ~6k токенов). Читать ТОЛЬКО когда нужны конкретные значения / код / таблицы по конкретному паттерну. Для принципов и быстрого review достаточно обзорного файла.

**Использовать этот файл ТОЛЬКО с явного разрешения пользователя** (те же триггеры, что и для обзорного файла: «use Deep Styles», «используй Deep Styles», «использовать глубокие стили», «применить deep styles», «глубокие стили»).
