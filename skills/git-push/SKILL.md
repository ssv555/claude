...
2. **Code Review**: Launch the `code-reviewer` agent to review all changes.
   ...
   - If agent returns issues → show them and call AskUserQuestion:
     ```json
     {
       "questions": [{
     ...
     ```
   - If user chose "Остановить" → output "Push отменён. Исправьте замечания и вызовите /git-push снова." and STOP.
