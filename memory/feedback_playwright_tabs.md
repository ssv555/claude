...
**Why:** (incident 2026-04-06) proposed `taskkill` on chrome.exe to "unstick Playwright" — but 2–4 concurrent Claude sessions share the one live `claude-def` browser, and any "cleanup" wipes the user's + other sessions' tabs. Browser lifetime belongs to the user, period.
...
**Why:** (incidents) 2026-04-05 — two near-identical `bunx @playwright/mcp` entries (`playwright`+`playwright_iso`) raced on the shared Windows bunx temp dir → both Failed; tabs would have solved it (the `_iso` entry only existed because I hadn't mentioned tabs). 2026-04-06 — skipped `browser_tabs list`, hit the lock, then escalated wrongly (WebFetch → screen_capture → taskkill), each step worse. Only correct lock response: name the profile, wait for the user.
